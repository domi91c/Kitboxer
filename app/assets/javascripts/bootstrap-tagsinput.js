(function(global, factory) {
  ...
  var BACKSPACE = 8, TAB = 9, ENTER = 13, LEFT = 37, RIGHT = 39, DELETE = 46,
      COMMA = 188
  var SEPERATOR = ','
  ...
  function tagsInput(input) {
    ...
    function $(selector, all) {
      return all === true ? Array.prototype.slice.call(
          base.querySelectorAll(selector)) : base.querySelector(selector)
    }
    ...
    function setValue(value) {
      $('.tag', true)
          .forEach(function(t) {return base.removeChild(t)})
    ...
    function addTag(text) {
      ...
      if (!input.getAttribute('duplicates')) {
        var _ret = (function() {
          ...
          if (exisingTag) {
            ...
            setTimeout(function() {return exisingTag.classList.remove('dupe')},
                100)
            return { v: false }
          }
        })()
      ...
      base.insertBefore(createElement('span', 'tag', tag, { tag: tag }),
          base.input)
    }
    ...
    function setInputWidth() {
      ...
      base.input.style.width = Math.max(
          w - (last ? last.offsetLeft + last.offsetWidth : 5) - 5, w / 4) +
          'px'
    }

    function savePartialInput(value) {
      if (typeof value !== 'string' &&
          !Array.isArray(value)) {value = base.input.value}
    ...
    base.input.addEventListener('keydown', function(e) {
      var el = base.input, key = e.keyCode || e.which,
      ...
      if (key === ENTER || key === COMMA || key === TAB) {
        ...
        if (selectedTag.nextSibling !== base.input) select(
            selectedTag.nextSibling)
        ...
        if (selectedTag) {
          ...
          setInputWidth()
          save()
        } else if (last && pos === 0) {select(last)} else {return}
      } else if (key === LEFT) {
        if (selectedTag) {
          if (selectedTag.previousSibling) {
            select(selectedTag.previousSibling)
          }
        } else if (pos !== 0) {return} else {select(last)}
      } else if (key === RIGHT) {
    ...
    base.input.addEventListener('paste',
        function() {return setTimeout(savePartialInput, 0)})
